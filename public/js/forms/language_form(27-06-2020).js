...
*/
	...
	$(document).on('click', '#remove', function(){
		...
		//$('#language_action_button').attr('disabled', true);	
		//$('#languages_name').prop("disabled",true);
		//var language = $('#languages_name').val();
		//var count = $(this).parent().parent().length;
		//var count = $('#mytbody tr').length;
		//alert(count);return;
   		...
   		$.ajax({
   			...
   			success:function(html){
				...
				$.each(array,function(i){
					...
					//if(array.length > 1){				   
					/*var html1 = '<tr>';
					html1 += '<td><input type="text" name="languages_name[]" id="languages_name" class="form-control language_list" value="'+array[i]+'"/></td>';	
								
					 if(array.length >= 1){
					 	html1+= '<td><button type="button" name="edit_remove" id="edit_remove" class="btn btn-danger">-</button></td></tr>';
					}
					else {
						html1+='<td><button type="button" name="edit_add" id="edit_add" class="btn btn-success">+</button></td></tr>';
					}*/
   	 				...
   	 				/*else if(array.length == 1){
   	 					var tr ='<tr>'+
			   	 			'<td class="td1"><input type="text" name="languages_name[]" id="languages_name" class="form-control languages_name_list" placeholder="Add language name" /></td>'+
			   	 			'<td class="td1"><button type="button" name="add" id="add" class="add btn btn-success"><span class="glyphicon glyphicon-plus"></span></button></td>'+
			   	 			'</tr>';
			   	 			$('#language_field').append(tr);
			   	 			$('#languages_name').val(html.data.languages_name[0]);

   	 			   }else {
   	 				var tr ='<tr>'+
			   	 			 '<td class="td1">BOOK NAME</td>'+
			   	 			 '<td class="td1"><button type="button" name="add" id="add" class="add btn btn-danger"><span class="glyphicon glyphicon-plus"></span></button></td>'+
			   	 			'</tr>';
			   	 			$('#language_field').html(tr);
   	 			  }*/
		      ...
		      /*  if(array.length==1){			
					...
					//$('#languages_name').prop("disabled",true);
					$('#language_action_button').attr('disabled', true);
					alert('does not delete all entry');
					return;			
				}	*/	
/*var count = $('#mytbody tr').length;
	if($('#mytbody tr').length==2){
